...
function predict_tws(studies::Dict{String, DataFrame})
    ...
    group = [:Item, :Condition, :Subject, :Electrode, :z_Zipf_freq,
            :z_Tw_position, :z_leo13b_surp, :z_gerpt2_surp, :z_gerpt2large_surp, :assoc_cond, :exp_cond]

    ##### Using frequency and target word position as fixed effects ####
    ...
    #n4_cond_f = @formula N400_mean ~ assoc_cond + exp_cond + z_Zipf_freq + z_Tw_position + (1|Item) + (1|Subject) + (1|Electrode)
    ...
    #p6_cond_f = @formula P600_mean ~ assoc_cond + exp_cond + z_Zipf_freq + z_Tw_position + (1|Item) + (1|Subject) + (1|Electrode)
    ...
    #### Using Item as fixed effect ####
    # Define LME formulas for time windows
    # N400
    n4_null_f = @formula N400_mean ~ Item + (1|Subject) + (1|Electrode)
    #n4_cond_f = @formula N400_mean ~ Condition + z_Zipf_freq + z_Tw_position + (1|Item) + (1|Subject) + (1|Electrode)
    n4_cond_f = @formula N400_mean ~ assoc_cond + exp_cond + Item + (1|Subject) + (1|Electrode)
    n4_leo13b_f = @formula N400_mean ~ z_leo13b_surp + Item + (1|Subject) + (1|Electrode)
    n4_gerpt2large_f = @formula N400_mean ~ z_gerpt2large_surp + Item + (1|Subject) + (1|Electrode)
    n4_gerpt2_f = @formula N400_mean ~ z_gerpt2_surp + Item + (1|Subject) + (1|Electrode)
    n4_formulas = [("null", n4_null_f), ("condition", n4_cond_f), ("leo13b", n4_leo13b_f), ("gerpt2large", n4_gerpt2large_f), ("gerpt2", n4_gerpt2_f)]

    # P600
    p6_null_f = @formula P600_mean ~ Item + (1|Subject) + (1|Electrode)
    #p6_cond_f = @formula P600_mean ~ Condition + z_Zipf_freq + z_Tw_position + (1|Item) + (1|Subject) + (1|Electrode)
    p6_cond_f = @formula P600_mean ~ assoc_cond + exp_cond + Item + (1|Subject) + (1|Electrode)
    p6_leo13b_f = @formula P600_mean ~ z_leo13b_surp + Item + (1|Subject) + (1|Electrode)
    p6_gerpt2large_f = @formula P600_mean ~ z_gerpt2large_surp + Item + (1|Subject) + (1|Electrode)
    p6_gerpt2_f = @formula P600_mean ~ z_gerpt2_surp + Item + (1|Subject) + (1|Electrode)
    p6_formulas = [("null", p6_null_f), ("condition", p6_cond_f), ("leo13b", p6_leo13b_f), ("gerpt2large", p6_gerpt2large_f), ("gerpt2", p6_gerpt2_f)]
    ...
    for (study_id, df) in studies
        ...
        # Code Condition
        assoc_code = Dict("A" => -0.5, "B" => 0.5, "C" => -0.5, "D" => 0.5)
        exp_code = Dict("A" => -0.5, "B" => -0.5, "C" => 0.5, "D" => 0.5)
        ...
        for (window, interval) in time_windows[study_id]
            ...
            println("Contrast coding association")
            df_window = transform(df_window, :Condition => ByRow(x ->  assoc_code[x]) => :assoc_cond)
            println("Contrast coding expectancy")
            df_window = transform(df_window, :Condition => ByRow(x ->  exp_code[x]) => :exp_cond)

            df_window_g = groupby(df_window,group)
            df_window_c = combine(df_window_g, ws => mean)
            print(first(df_window,10))

            CSV.write("../data/$(study_id)/$(study_id)_$(window)_mean_test.csv",df_window_c)
            ...
            open("../results/erp_aic/$(study_id)_$(window)_lme_test_item.txt", "w") do file
...
CSV.write("../results/erp_aic/aic_diffs_adsbc21test_item.csv", aic_df)